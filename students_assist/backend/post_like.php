...
$resu = $db->prepare('SELECT * FROM post_like_table WHERE  postid=:postid and userid=:userid');
...
$statement = $db->prepare('INSERT INTO post_like_table
...
$result = $db->prepare('SELECT * FROM posts_table WHERE id =:id');
...
$statement2 = $db->prepare('INSERT INTO notification_table
...
$cct = $db->prepare('select * from posts_table where id=:id');
$cct->execute(array(':id' =>$post_id));
$rct_row = $cct->fetch();
$totallikes = $rct_row['total_like'];
$total_l = $totallikes + 1;

$update2= $db->prepare('UPDATE posts_table set total_like =:total_like where id=:id');
...
//$resultp = $db->prepare('SELECT COUNT(*) AS cnt FROM post_like_table WHERE postid=:postid');
$resultp = $db->prepare('SELECT COUNT(*) AS cnt FROM post_like_table WHERE  postid=:postid');
