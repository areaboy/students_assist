...
$cctx = $db->prepare('select * from users_table where userid =:userid');
...
$res= $db->prepare("SELECT count(*) as totalcount FROM posts_table WHERE userid=:userid");
...
$result = $db->prepare("SELECT * FROM posts_table WHERE userid=:userid order by id desc limit :row1, :rowpage");
...
<span style='color:fuchsia;'><b> Points Awarded So Far for your Contributions: </b> <span class='point_count'><?php echo $points; ?> (Points)</span></span> <br>
